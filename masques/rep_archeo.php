		...
		for ($i=0; $i < count($tableau); $i++) {
			...
			// traitement des champs personnes
				if ($key == 'AUTEUR' || $key == 'UTILISATEUR' || $key == 'COLLECTEUR' || $key == 'INVENTEUR' || $key == 'PROPRIETAIRE' || $key == 'COMMISSAIRE_PRISEUR' || $key == 'GALERIE' || $key == 'DEPOSITAIRE' || $key == 'ATTRIBUTION' || $key == 'ATTRIBUTEUR' || $key == 'SERVICE_GESTIONNAIRE' || $key == 'ANCIENNE_APPARTENANCE' || $key == 'SERVICE_GESTIONNAIRE' || $key == 'ANCIEN_DEPOSITAIRE' || $key == 'DESCRIPTEUR') {
					...
					($premier) ? $requete = $requete." INDEX_OBJET = '".$noindex."' " : $requete = $requete." ".$operateur." INDEX_OBJET = '".$noindex."' ";
					$premier = false;
				} else {
...
/* Requète */
mysql_select_db($database_alienorweblibre, $alienorweblibre);
$query_objet = "SELECT INDEX_OBJET, DENOMINATION, TYPOLOGIE, NUMERO_INVENTAIRE, LOCALISATION FROM objet ";
if ($vide == 0) {
...
?>
<div id="haut">
<h2>R&eacute;sultat d'une recherche avec le masque Arch&eacute;ologie </h2>
</div>
    ...
    <form name="Selection" action="rep_ethno.php">
        <table width="90%" style="border: 1px solid #666666" cellpadding="3">
            <tr>
                ...
                <th scope="col" width="23%">Typologie</th>
                <th scope="col" width="23%">Site</th>
                <th scope="col" width="23%">Inventeur</th>
		...
		?>
                <tr>
                    <td align="center" valign="middle" class="<?php echo $color; ?>"><input name="sel" type="checkbox" value="<?php echo $row_objet['INDEX_OBJET']; ?>"></td>
                    ...
                    <td class="<?php echo $color; ?>"><?php echo $row_objet['TYPOLOGIE']; ?></td>
...
$i = 0 ;
	do {
		...
		$i++;
                    ...
                    <td class="<?php echo $color; ?>"><?php
mysql_select_db($database_alienorweblibre, $alienorweblibre);
$query_inventeur = "SELECT ETAT_CIVIL FROM personne,obj_per WHERE obj_per.INDEX_OBJET =".intval($row_objet['INDEX_OBJET'])." AND personne.INDEX_PERSONNE = obj_per.INDEX_PERSONNE AND obj_per.QUALIFIANT = 'INVENTEUR' ORDER BY INDEX_OBJ_PER ASC";
$inventeur = mysql_query($query_inventeur, $alienorweblibre) or die(mysql_error());
$row_inventeur = mysql_fetch_assoc($inventeur);
$totalRows_inventeur = mysql_num_rows($inventeur);
$i = 0 ;
	do {
		if ($i !=0 ) {
		echo "/";
		}
		echo $row_inventeur['ETAT_CIVIL'];
		$i++;
		} while ($row_inventeur = mysql_fetch_assoc($inventeur)); ?></td>
