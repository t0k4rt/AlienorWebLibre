...
// Début affichage de tous les champs de la fiche
	...
	$query_rech_personne = "SELECT * FROM personne WHERE INDEX_PERSONNE = ".$noFiche;
	$rech_personne = mysql_query($query_rech_personne, $alienorweblibre) or die(mysql_error());
	$row_rech_personne = mysql_fetch_assoc($rech_personne);
	$totalRows_rech_personne = mysql_num_rows($rech_personne);	
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == 'personne')) {
	$noFiche = intval($_POST['INDEX_PERSONNE']);
	$totalRows_rech_doublon = 0;
	$requete = "";	
	if ($_POST['ETAT_CIVIL'] != $_POST['ETAT_CIVIL_INIT']) {	
		// Recherche si doubon de numéro d'inventaire sur un même musée
		mysql_select_db($database_alienorweblibre, $alienorweblibre);
		$query_rech_doublon = "SELECT * FROM personne WHERE ETAT_CIVIL = '".addslashes($_POST['ETAT_CIVIL'])."'";
	...
	if ($totalRows_rech_doublon != 0) {	
		$msg = "Cet état civil existe déjà";
		$doublon = 1;
	} else {
		if ($noFiche == 0 && $_POST['ETAT_CIVIL'] != '') {
			// Début création de fiche */
			// Création de fiche personne
			$IDENTIFIANT_national = ident_nat("personnes");
			$insertSQL = sprintf("INSERT INTO personne (ETAT_CIVIL,FICHE_CREEE_LE,IDENTIFIANT_NATIONAL) VALUES (%s,%s,%s)",
								GetSQLValueString($_POST['ETAT_CIVIL'], "text"),
			...
			mysql_select_db($database_alienorweblibre, $alienorweblibre);
			$query_personnes = "SELECT INDEX_PERSONNE FROM personne WHERE ETAT_CIVIL = '".$_POST['ETAT_CIVIL']."'";
			$personnes = mysql_query($query_personnes, $alienorweblibre) or die(mysql_error());
			$row_personnes = mysql_fetch_assoc($personnes);
			$totalRows_personnes = mysql_num_rows($personnes);
			do {
				$noFiche = intval($row_personnes['INDEX_PERSONNE']);
			} while ($row_personnes = mysql_fetch_assoc($personnes));
		...
		foreach($_POST as $key=>$val) {
			...
			$val = trim($val);
			$val = addslashes($val);
			if ($key != 'valider_x' && $key != 'valider_y' && $key != 'MM_update' && $key != 'ETAT_CIVIL_INIT' ) {	
			...
			$tableau = split("/",$val);
				...
				for ($i=0; $i < count($tableau); $i++) {
					...
					if ($key == 'LIEU_NAISSANCE' || $key == 'LIEU_DECES' || $key == 'LIEU_TRAVAIL' || $key == 'LIEU_RESIDENCE' || $key == 'GALERIE') {
						// Recherche des lieux
						mysql_select_db($database_alienorweblibre, $alienorweblibre);
						$query_liaison_debut = "SELECT INDEX_LIEU FROM per_lie WHERE QUALIFIANT = '".$key."' AND INDEX_PERSONNE = ".$noFiche."";
						...
						for ($i=0; $i < count($tableau); $i++) {
							...
							if ($lieu_exe_trouve == "" && $val != "") {
								...
								$insertSQL = sprintf("INSERT INTO per_lie (INDEX_PERSONNE,INDEX_LIEU,QUALIFIANT) VALUES (".$noFiche.",".$lieu_exe_trouve.",'".$key."')");
								mysql_select_db($database_alienorweblibre, $alienorweblibre);
								$Result1 = mysql_query($insertSQL, $alienorweblibre) or die(mysql_error());
								...
								mysql_select_db($database_alienorweblibre, $alienorweblibre);
								$query_liaison = "SELECT INDEX_PER_LIE FROM per_lie WHERE INDEX_LIEU = ".$lieu_exe_trouve." AND INDEX_PERSONNE = ".$noFiche." AND QUALIFIANT = '".$key."'";
								...
								do {
									$index_liaison = intval($row_liaison['INDEX_PER_LIE']);
								...
								if ($index_liaison == "" && $val != "") {
									// Début liaison de la fiche lieu avec la fiche personne
									$insertSQL = sprintf("INSERT INTO per_lie (INDEX_PERSONNE,INDEX_LIEU,QUALIFIANT) VALUES (".$noFiche.",".$lieu_exe_trouve.",'".$key."')",
									GetSQLValueString($_POST['INDEX_PERSONNE'], "num"));
									mysql_select_db($database_alienorweblibre, $alienorweblibre);
									$Result1 = mysql_query($insertSQL, $alienorweblibre) or die(mysql_error());
									// Fin liaison de la fiche lieu avec la fiche personne
								}
						...
						while (list($keyb,$valb)=each($difference)) {
							if ($valb != 0) {
								$deleteSQL = sprintf("DELETE FROM per_lie WHERE INDEX_LIEU = ".intval($valb)." AND INDEX_PERSONNE = ".$noFiche." AND QUALIFIANT = '".$key."'");
								mysql_select_db($database_alienorweblibre, $alienorweblibre);
					...
					// ----------------- Fin traitement des champs lieux -----------------
						...
						if ($key == 'BIBLIOGRAPHIE' || $key == 'PHOTOGRAPHIE' || $key == 'EXPOSITION' || $key == 'CEDEROM' || $key == 'INTERNET' || $key == 'TAPUSCRIT' || $key == 'MANUSCRIT' || $key == 'VIDEO' || $key == 'REPRODUCTION') {
							...
							mysql_select_db($database_alienorweblibre, $alienorweblibre);
							$query_liaison_debut = "SELECT INDEX_DOCUMENTATION FROM per_doc WHERE QUALIFIANT = '".$key."' AND INDEX_PERSONNE = ".$noFiche;
							$liaison_debut = mysql_query($query_liaison_debut, $alienorweblibre) or die(mysql_error());
							$row_liaison_debut = mysql_fetch_assoc($liaison_debut);
							$totalRows_liaison_debut = mysql_num_rows($liaison_debut);
							$execution_liaison_debut = array();
							...
							for ($i=0; $i < count($tableau); $i++) {
								...
								if ($docum_exe_trouve == "" && $val != "") {
									...
									$insertSQL = sprintf("INSERT INTO per_doc (INDEX_PERSONNE,INDEX_DOCUMENTATION,QUALIFIANT) VALUES (".$noFiche.",".$docum_exe_trouve.",'".$key."')");
									mysql_select_db($database_alienorweblibre, $alienorweblibre);
									$Result1 = mysql_query($insertSQL, $alienorweblibre) or die(mysql_error());
									...
									// Recherche si liaison existante
									mysql_select_db($database_alienorweblibre, $alienorweblibre);
									$query_liaison = "SELECT INDEX_PER_DOC FROM per_doc WHERE INDEX_DOCUMENTATION = ".$docum_exe_trouve." AND INDEX_PERSONNE = ".$noFiche." AND QUALIFIANT = '".$key."'";
									$liaison = mysql_query($query_liaison, $alienorweblibre) or die(mysql_error());
									$row_liaison = mysql_fetch_assoc($liaison);
									$totalRows_liaison = mysql_num_rows($liaison);
									do {
										$index_liaison = $row_liaison['INDEX_PER_DOC'];
									} while ($row_liaison = mysql_fetch_assoc($liaison));	
									// Fin de recherche
							...
							while (list($keyc,$valc)=each($difference)) {
								if ($valc != 0) {
									$deleteSQL = sprintf("DELETE FROM per_doc WHERE INDEX_DOCUMENTATION=".intval($valc)." AND INDEX_PERSONNE=".$noFiche." AND QUALIFIANT = '".$key."'");
									mysql_select_db($database_alienorweblibre, $alienorweblibre);
									$Result1 = mysql_query($deleteSQL, $alienorweblibre) or die(mysql_error());
								}
							}				
							...
							if ($requete == "") {
								if ($key != 'INDEX_PERSONNE'){$requete = $requete." ".$key." = '".$val."'";};
							} else {
		...
		$updateSQL = sprintf("UPDATE personne SET ".$requete." WHERE INDEX_PERSONNE = ".$noFiche);
